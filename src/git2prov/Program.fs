...
let writeProv r showWorking since =
...
let main argv = 
  ...
  match args.GetResult (<@ Server @>,defaultValue=None) with
    | None ->
      writeProv repo showWorking since
    | Some port ->
      Http.serve repo port

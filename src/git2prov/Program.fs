...
with interface IArgParserTemplate with member s.Usage =
  match s with
    ...
    | ShowCompilation _ -> "Generate a compilation activity for included git history"
...
let main argv = 
  ...
  match args.GetResult (<@ Server @>,defaultValue=0) with
    | 0 -> gatherProv repo includeWorking since
